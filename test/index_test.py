...
class TestSQLMoreQueries(unittest.TestCase):
    ...
    def test_select_hungry_dogs_name_and_breed_ordered_by_oldest_to_youngest(self):
        result = [('Snoopy', 'beagle'), ('Clifford', 'big red'), (None, 'golden retriever'), ('Scooby', 'great dane'), ('Lassie', 'collie'), ('Pickles', 'black lab')]
        self.assertEqual(table.execute(select_hungry_dogs_name_and_breed_ordered_by_oldest_to_youngest()).fetchall(), result)

    def test_select_name_and_age_of_oldest_dog(self):
        result = [('Pickles', 13)]
        self.assertEqual(table.execute(select_name_and_age_of_oldest_dog()).fetchall(), result)
    ...
    def test_select_name_and_temperament_of_dogs_between_age_five_and_ten_ordered_by_oldest_to_youngest(self):
        result = [('McGruff', 'bloodhound'), ('Snowy', 'fox terrier'), ('Lassie', 'collie'), ('Scooby', 'great dane'), ('Little Ann', 'coonhound')]
        self.assertEqual(table.execute(select_name_and_temperament_of_dogs_between_age_five_and_ten_ordered_by_oldest_to_youngest()).fetchall(), result)

    def test_select_name_and_age_of_hungry_dogs_between_age_two_and_seven_in_alphabetical_order(self):
        result = [(None, 4, 1), ('Clifford', 4, 1), ('Lassie', 7, 1), ('Scooby', 6, 1), ('Snoopy', 3, 1)]
        self.assertEqual(table.execute(select_name_and_age_of_hungry_dogs_between_age_two_and_seven_in_alphabetical_order()).fetchall(), result)
